...
while True:
    ...
    InputRoute = './images/Image_'+str(p)+'.jpg' # Esto nomás es la ruta para la img, pq no me dejaba ponerla directamente en el model.predict, pq lo toma como más de 1 parámetro y se enoja
    ...
    model.predict("./images/IMG_20230208_135811_jpg.rf.a9ca0a33007f400de4a41cbe8acf2dad.jpg", confidence=30, overlap=30).save(ImgNewName)

    prediction = model.predict("./images/IMG_20230208_135811_jpg.rf.a9ca0a33007f400de4a41cbe8acf2dad.jpg", confidence=30, overlap=30).json().get('predictions')

    print(prediction)

    # cv2.imshow('IAImgInput', ImgNewName)
    ...
    for i in range(len(prediction)):
        ...
        localizacion.append({'x': int(prediction[i].get('width')) / 2 + int(prediction[i].get('x')), 'y': int(prediction[i].get('height')) / 2 + int(prediction[i].get('y'))})
    ...
    if not localizacion == []: direccion = find_leftmost_section(localizacion)
